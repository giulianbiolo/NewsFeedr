        ...
        <tr v-if="feed_data.data" v-for="feed in feed_data.data" class="hover" :href="feed.link">
          <td>
            ...
            <div class="btn btn-ghost btn-square" @click="toggleBookmark(feed.link)">
              <BookmarkIcon class="h-6 w-6" :class="isBookmarked(feed.link) ? 'text-yellow-500' : 'text-gray-500'" />
            </div>
          ...
          <td class="w-80">
            <div class="font-bold">{{ feed.title.length > 150 ? feed.title.slice(0, 150) + "..." : feed.title }}</div>
